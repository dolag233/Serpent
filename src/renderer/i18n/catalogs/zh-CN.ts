...
export const zhCN = {
  ...
  dialog: {
    ...
    about: {
      ...
      updateAvailable: "Serpent {version} 已可用。",
  ...
  settings: {
    ...
    mcpPermissionClientHint: "模式会跨应用重启保留，仅作用于这个客户端凭据。",
    mcpAccessMode: "访问模式",
    ...
    mcpConfigHint: "创建凭据，并将包含服务地址和 Bearer Token 的配置复制到剪贴板。",
    mcpConfigFormat: "配置格式",
    mcpConfigGeneric: "通用 JSON",
    mcpConfigClaude: "Claude",
    mcpConfigCursor: "Cursor",
    mcpConfigCodex: "Codex",
    mcpConfigEndpoint: "地址 + Token",
    mcpCopyConfig: "复制配置",
    mcpConfigCopied: "MCP 配置已复制到剪贴板。",
    mcpCredentialsTitle: "客户端凭据",
    mcpCredentialsHint: "Token 只保存在 Serpent 的私有用户数据目录中；客户端不再需要时请及时撤销。",
    mcpRevoke: "撤销",
    mcpRevoked: "已撤销",
    mcpNoCredentials: "尚未创建客户端凭据。",
  ...
  toast: {
    ...
    openRecentFailed: "无法打开最近使用的资源库。文件夹可能已移动、磁盘已断开，或数据库无法在网络盘上打开。请从「打开资源库」重新选择，或把库复制到本机磁盘。",
  ...
  error: {
    code: {
      ...
      INVALID_LIBRARY_PATH: "所选位置不能作为资源库文件夹。请选择本机磁盘上已存在且可写入的文件夹，不要选择文件或已断开的磁盘。",
      ...
      LIBRARY_NOT_WRITABLE: "无法在所选文件夹写入文件。可能是没有写权限、磁盘已满或只读，或文件被其他程序占用。请改选本机磁盘上可写入的文件夹后重试。",
      LIBRARY_NETWORK_SHARE: "无法把资源库数据库放在此网络共享（NAS/SMB）上。SQLite 需要本机磁盘的文件锁，多数共享盘不提供。请在本机磁盘创建或复制资源库；若需在多台电脑间同步，请改用 WebDAV 同步。",
      LIBRARY_BUSY: "该资源库正被另一个 Serpent 窗口占用，或数据库暂时锁定。请等待几秒后重试；不要在两台电脑上同时打开同一资源库。",
      LIBRARY_CLEANUP_FAILED: "创建资源库失败，且临时文件无法自动清理。请删除所选位置旁的 `.serpent-create-*.partial` 文件夹，然后换一个本机可写文件夹重试。",
