...
export const zhCN = {
  ...
  settings: {
    ...
    pluginSafeMode: "安全模式",
    pluginSafeModeHint: "暂时停用无限制模式插件；受限模式插件可继续运行。不会卸载或删除任何插件。",
    pluginInstall: "安装插件",
    pluginInstallHint: "安装已经编译好的本地文件夹、ZIP，或已包含成品的 GitHub 仓库。",
    ...
    pluginGitHubRepository: "GitHub 仓库链接",
    ...
    pluginRuntimeTrusted: "无限制模式",
    pluginRuntimeTrustedHint: "无限制模式使用完整 Node.js 能力，可读写本机文件、访问网络并启动其他程序，风险更高。声明的权限只能约束经 Serpent Gateway 的命令，不能当作系统沙箱。请确认来源后再谨慎使用。",
    pluginRuntimeStandard: "受限模式",
    pluginRuntimeStandardHint: "受限模式在独立进程的沙箱中执行，只能调用你授权的 Gateway 命令。",
    pluginTrustTrustedConfirmHint: "信任后，此无限制模式插件将获得本机完整系统能力，请谨慎确认。",
    pluginThemeTrustedCssHint: "此插件可向 Serpent 宿主界面注入自定义 CSS。宿主 CSS 可能破坏布局、无障碍与升级兼容性。",
    pluginThemeTrustedCssTrustHint: "无限制模式下的宿主 CSS 可修改插件 iframe 之外的 Serpent 界面。仅信任你接受其样式变更的插件。",
    ...
    pluginStatusSafeMode: "已被安全模式停用（无限制模式）",
