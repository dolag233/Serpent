...
export function generatePluginSdkTypeDeclaration(_moduleSpecifier = '@serpent/plugin-api'): string {
  ...
  return [
    ...
    '  }',
    '',
    '  interface SerpentPluginWidgetState<T> {',
    '    get(): T;',
    '    set(value: T): void;',
    '  }',
    '',
    '  interface SerpentPluginWidgetToolkit {',
    '    state<T>(initial: T): SerpentPluginWidgetState<T>;',
    '    column(...children: readonly unknown[]): unknown;',
    '    row(...children: readonly unknown[]): unknown;',
    ...
    '    switch(spec: Readonly<Record<string, unknown>>): unknown;',
    '    toggle(spec: Readonly<Record<string, unknown>>): unknown;',
    '    slider(spec: Readonly<Record<string, unknown>>): unknown;',
    '  }',
    '',
    ...
    '    readonly invocation?: { readonly contextId: string; readonly revision: number; readonly libraryId: string; readonly app?: { readonly locale: string }; readonly selection: { readonly ref?: string; readonly refs: readonly string[]; readonly assetIds: readonly string[]; readonly folderIds: readonly string[]; readonly collectionIds: readonly string[]; readonly assets?: readonly { readonly id: string; readonly name: string; readonly relativeFilePath: string; readonly mediaType: string; readonly byteSize: number; readonly durationMs?: number | null; readonly currentRevisionId?: string; /* Host must populate currentRevisionId for write-back; pass it as expectedRevisionId. */ readonly folderId?: string | null; readonly locationKind: \'managed\' | \'linked\'; readonly linkedFolderId?: string | null }[] }; readonly browse: Readonly<Record<string, unknown>>; readonly viewer: { readonly active: boolean; readonly assetId?: string } };',
    '  }',
    '',
    ...
    '      readContent(assetId: string, options?: { readonly maxBytes?: number; readonly offsetBytes?: number }): Promise<{ readonly assetId: string; readonly revisionId: string; readonly byteSize: number; readonly dataBase64: string; readonly truncated: boolean; readonly mimeType: string | null }>;',
    ...
    '      renameFile(assetId: string, newBaseName: string, options?: { readonly fileName?: string }): Promise<unknown>;',
    ...
    '        readonly render: (ui: SerpentPluginWidgetToolkit) => unknown;',
