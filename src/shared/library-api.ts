...
export interface SerpentLibraryApi {
  ...
  fetchBrowseSessionPage(input: { libraryId: string; sessionId: string; limit?: number; offset?: number }): Promise<LibraryApiResult<{ sessionId: string; changeSequence: number; catalogSequence?: number; items: AssetSummary[]; total: number; offset: number; snippets?: { assetId: string; text: string }[] } | { stale: true; sessionId: string; reason: 'library-generation' | 'change-sequence' | 'catalog-sequence' | 'missing' }>>;
  fetchBrowseSessionGeometry(input: { libraryId: string; sessionId: string; startIndex: number; limit?: number }): Promise<LibraryApiResult<(BrowseGeometryBlock & { catalogSequence?: number }) | { stale: true; sessionId: string; reason: 'library-generation' | 'change-sequence' | 'catalog-sequence' | 'missing' }>>;
