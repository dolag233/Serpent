...
export interface SerpentLibraryApi {
  ...
  fetchBrowseSessionGeometry(input: { libraryId: string; sessionId: string; startIndex: number; limit?: number }): Promise<LibraryApiResult<(BrowseGeometryBlock & { catalogSequence?: number; snapshotGeneration?: number | null }) | { stale: true; sessionId: string; reason: 'library-generation' | 'change-sequence' | 'catalog-sequence' | 'missing' }>>;
