...
只读探索 `Explore Inspector multi-select` 与落地提交 `c1e0c87` 对照：标量聚合、标签交集、去掉 `applyToSelection`、多选元数据加载与批量写回均已覆盖。未采纳项（显式保留）：专用 worker 批量 description/favorite/sourceUrl API（当前按资产循环 `setAssetMetadata` + 冲突跳过）；英雄区仍显示主资产预览。
...
2026-07-18 用户确认 `SELECT-011` 通过（基线 `5104747`）：主选定框裁剪 + 轻微错开，标题不被遮挡。
