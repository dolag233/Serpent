...
- `Serpent-mxxc`：回收站删除成功后记录可撤销的资产集合，通知面板使用无边框回撤图标（可访问名称为“撤销”）；撤销复用现有恢复/冲突处理流程，成功恢复后按当前浏览范围刷新视图并清除撤销状态；硬盘删除会清除不可用的撤销状态。
...
## 人工复验
 ...
 Windows 真实应用仍需复验：合集 F2/右键重命名的行内输入、所有相关快捷键标签；删除资产后通知面板应显示无边框回撤图标，点击后当前浏览范围立即刷新并恢复原位置，遇到冲突时进入已有冲突处理。

## 2026-08-01 复验纠正

用户明确指出上一版反馈记录存在两处误读：硬盘删除问题不是“只需关闭查看器并重试”，而是资产和含资产文件夹均高概率收到权限错误；瀑布流也不能改成 CSS Grid。当前修复因此拆成两层：Renderer 在硬盘删除前清掉选中/悬停/查看器媒体并等待卸载，Worker 取消并等待目标资产的缩略图/代理生成任务；布局代码恢复到 `d2774f9^` 的 `distributeMasonryItems` 显式列布局。2026-08-01 用户已验收硬盘删除/媒体句柄与瀑布流恢复；通知撤销入口继续按图标样式收口。
