...
| 自动化执行（Automation Execution） | 一次脚本运行或 MCP 自动化会话形成的受控执行单元；绑定来源、代码哈希、单一目标资源库、能力、资源预算、状态和日志。避免称为系统进程或后台 Job，后两者是实现/调度概念。 | 已确认 |
...
| 标准插件 | 运行在受控 JS 环境、只能使用声明并授权的 Plugin SDK 的插件；权限是 Host 可执行的能力边界。 | 已确认 |
| 可信插件 | 由用户明确选择、运行在独立 Node.js 进程并具有完整系统能力的插件；权限清单用于风险披露和 Serpent API 管理，不承诺拦截全部系统行为。 | 已确认 |
