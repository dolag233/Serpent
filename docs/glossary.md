...
| Label | 用户或 AI 为资产设置的展示名称。存在 Label 时，界面优先显示 Label，而非原始文件名。 | 已确认 |
...
| 搜索 | 对 Label、文件名、标签、描述、源链接、文件夹和其他元信息执行的加权全文检索；不同于结构化过滤和排序。 | 已确认 |
| AI 查询转换 | 快速模型把自然语言转换成关键词、同义词、排除词和结构化过滤条件，再交给普通搜索引擎执行。 | 已确认 |
| 向量搜索 | 使用 embedding 向量计算语义或视觉相似度的搜索方式；不进入 MVP。 | 已确认 |
| 人工内容 | 用户填写或修改的 Label、描述、标签等资产信息；独立于 AI 内容保存并拥有显示优先级。 | 已确认 |
| AI 内容 | 模型生成的 Label、描述、标签或结构化元信息；与人工内容和文件提取元信息分开保存，记录模型来源，可以按资产范围清空且不得覆盖人工内容。重新分析只保留当前结果。 | 已确认 |
| AI 写入策略 | 全局设置中按字段控制是否允许 AI 写入 Label、标签、描述等内容；MVP 默认全部开启。 | 已确认 |
