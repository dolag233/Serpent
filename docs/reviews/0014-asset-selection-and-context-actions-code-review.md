# 切片 0014 双轴代码审查：资产选择与上下文操作（P0 右键菜单热修）

> 状态：双轴通过（0 HARD 违规，4 非阻断气味）；完整 P0 规格满足
> 日期：2026-07-14
...
- 框选（marquee select）：未实施。统一的 `ContextMenu` 组件基础设施不依赖框选功能，P1 可独立开发。
- 组合键模型（Ctrl/Shift/Command）：未实施。现有 `onContextMenu` 已预留 `selectedIdSet` 扩展点。
- 移除顶部批量条：未实施。多选 ops bar 仍在 App.tsx 中。
- 批量右键菜单（"已选择 N 项"）：未实施。`ContextMenuDescriptor.type === "asset"` 当前仅为单项设计；批量扩展可在 descriptor 中增加 `assetIds[]` 字段。
- 统一批量菜单视觉打磨：未实施。

### Spec 偏离分析

无不当偏离。P0 范围严格限定于右键菜单可靠性与统一下层基础设施，P1 的框选/组合键/批量菜单/视觉打磨未实施属于有意的范围控制，非规格遗漏。

## 审查后修复

- 死代码 `useSingleContextMenu` export 别名已移除（grep 确认无引用）。
- 3 项测试缺口已关闭：window blur、四角 viewport clamp、scope-change。
- 非阻断气味 4 项记录为 follow-up，不在本次修改。

## 结论

双轴通过：Standards 轴 0 HARD 违规（4 非阻断气味已记录），Spec 轴 P0 功能完整满足（P1 为有意的范围控制，架构安全）。代码未提交（working tree uncommitted）。macOS Computer Use 人工视觉 QA 与 Windows 平台行为仍未验证，切片整体尚未 accepted。

**非阻断 follow-up 列表（不阻断合并）**：
1. 抽取 `useCloseOnEvent` 共享 helper 减少 5 套重复监听块
2. Backdrop Feature Envy：以 ref-based 方案替代 `querySelector(".context-menu")`
3. App.tsx 引号风格变更后续独立 commit
