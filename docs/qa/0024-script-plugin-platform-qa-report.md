...
| manifest 菜单树、二级/三级、before/after/first/last、快捷键 | `src/plugins/plugin-manifest.ts`、`src/plugins/plugin-contributions.ts`、`src/renderer/plugin-menu-contributions.ts` | manifest/registry/solver/shortcut 测试通过；菜单 Placement Solver 单测 11/11 通过，覆盖确定性排序、缺锚点、循环边和超深分支 | 当前 E2E 未逐项验证真实原生菜单视觉一致性；Computer Use 未执行 |
