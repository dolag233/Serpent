...
| manifest 菜单树、二级/三级、before/after/first/last、快捷键 | `src/plugins/plugin-manifest.ts`、`src/plugins/plugin-contributions.ts`、`src/renderer/plugin-menu-contributions.ts` | manifest/registry/solver/shortcut 测试通过 | 当前 E2E 未逐项验证真实原生菜单视觉一致性；Computer Use 未执行 |
