...
| UI-only 有界 Context、条件、隐藏/置灰/checked、多选 | `src/plugins/plugin-context.ts`、`src/renderer/plugin-contribution-context.ts`、`src/renderer/AssetContextMenu.tsx` | Context、browse/viewer、revision、菜单 solver、单/多/混合选择测试通过；旧 revision Predicate 取消/终态定向测试通过 | E2E 覆盖贡献列表/设置/重启恢复；真实右键点击与无选择/Viewer 全矩阵未执行 |
