...
| SELECT-011 | 多选 Inspector 堆叠预览与「等 n 个文件」 | 待人类验收 | 框选 ≥2 项带缩略图的资产，观察右侧 Inspector 顶部 | 预览框以主选图比例定尺寸，其余层同框裁剪（cover）并轻微错开；标题在堆叠下方完整可见，为「主文件名 等N个文件」 | [开发日志](../development/2026-07-18-ue-multi-edit-inspector-development-log.md) / [堆叠单测](../../tests/unit/inspector-multi-edit.test.ts) | 2026-07-18 实现；同日修正：绝对定位未撑高导致标题被盖住 → 主选定框 + cover 裁剪 + 小错位。 |
