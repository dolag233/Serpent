...
| SHELL-013 | 纯图标控件悬停提示 | 待人类验收 | 打开资源库后将指针悬停在侧栏「文件夹」旁的加号与链接图标；再悬停工具栏纯图标按钮、收起后的边缘展开按钮、Inspector 加标签/喜欢等 | 悬停出现与功能一致的原生 tooltip（如「添加文件夹」「导入链接文件夹」）；屏幕阅读器名称与提示文案一致 | [开发日志](../development/2026-07-18-icon-action-tooltips-development-log.md) / [attrs 单测](../../tests/unit/icon-action-attrs.test.ts) / [壳层 E2E 断言](../../tests/e2e/shell-navigation.test.ts)（本机 Electron 启动受 `--remote-debugging-port=0` 阻断，未当次绿） | — |
