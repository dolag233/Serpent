...
| SELECT-011 | 多选 Inspector 堆叠预览与「等 n 个文件」 | 待人类验收 | 框选 ≥2 项带缩略图的资产，观察右侧 Inspector 顶部 | 预览为最多 3 张卡片错位叠放（主选在最上）；标题为「主文件名 等N个文件」（N=选中总数） | [开发日志](../development/2026-07-18-ue-multi-edit-inspector-development-log.md) / [堆叠单测](../../tests/unit/inspector-multi-edit.test.ts) | 2026-07-18 实现。 |
