...
| SHELL-013 | 纯图标控件悬停提示 | 待人类验收 | 打开资源库后将指针悬停在侧栏「文件夹」旁的加号与链接图标；再悬停工具栏纯图标按钮、收起后的边缘展开按钮、Inspector 加标签/喜欢等 | 悬停约立刻出现与功能一致的提示气泡（如「添加文件夹」「导入链接文件夹」）；屏幕阅读器名称一致 | [开发日志](../development/2026-07-18-icon-action-tooltips-development-log.md) / [attrs 单测](../../tests/unit/icon-action-attrs.test.ts) / [壳层 E2E 断言](../../tests/e2e/shell-navigation.test.ts) | 2026-07-18 用户反馈 hover 无反应：根因为 Electron 拖拽区 + 原生 title 不可靠；已改为 `data-tooltip` CSS 提示并扩大 `no-drag` 覆盖。 |
...
| FOLDER-009 | 包含子文件夹显式开关 | 待人类验收 | 建「父文件夹」→子文件夹并导入资产到子文件夹；点开父文件夹；悬停并点击当前文件夹名旁的嵌套文件夹图标 | 默认父文件夹画布不显示子文件夹资产；图标旁悬停提示「包含子文件夹」；按下（高亮）后显示后代资产；「资源库根目录」与「所有资产」无此图标 | [开发日志](../development/2026-07-18-folder-include-subfolders-development-log.md) / [范围单测](../../tests/unit/folder-browse-scope.test.ts) / [E2E](../../tests/e2e/folder-recursive-scope.test.ts) | 2026-07-18 用户反馈：勿用文字勾选，改为展开语义图标并贴在当前文件夹名旁。 |
