...
| SHELL-020 | 壳层竖直对齐统一 | 待人类验收 | 打开资源库；观察顶栏（44px）、浏览标题栏、发现过滤条；对比图标/文字是否在各栏内垂直居中，相邻栏中线是否一致；看目录名旁数量徽标与过滤 chips | 各栏内图标与文字垂直居中；标题与数量徽标中线对齐；维度按钮与 chips 高度节奏一致；无偏上/偏下漂浮感 | [开发日志](../development/2026-07-18-shell-vertical-alignment-development-log.md) / 工单 Serpent-680（REQ-SHELL-002） | 2026-07-18 实现；Computer Use 未执行，移交人工 QA。 |
...
| COMMAND-003 | 工具栏视图/字段/更多工具经命令注册表 | 待人类验收 | 打开资源库；点平铺/瀑布流、文件名/大小/日期开关、刷新；再开「更多工具」点扩展/后台任务/AI | 行为与改前一致；无资源库时刷新禁用、「更多」仅扩展；导入仍只在资源库菜单 | [开发日志](../development/2026-07-18-toolbar-command-registry-development-log.md) / [toolbar 单测](../../tests/unit/toolbar-commands.test.ts) / 工单 Serpent-2rk | 2026-07-18 实现。 |
...
| META-009 | 编辑作者/自动提取 EXIF 作者 | 待人类验收 | ① 选中一张不带 EXIF 作者信息的图片，在 Inspector「作者」输入框填值并失焦保存，切换到其他资产再切回核对；再清空并保存核对已清空；② 另导入一张在其他工具（如 Photoshop/Lightroom）写入过「创建者/Artist」EXIF 或 IPTC 字段的图片，不做任何编辑，等待缩略图生成后查看 Inspector「作者」是否已自动填入该值；③ 框选一张已填作者与一张未填作者的资产，观察 Inspector 显示「多个值」 | ①手动输入的作者值保存后跨切换/重新选中保持一致，清空后变为空；②自动提取的资产在缩略图生成后作者字段自动填入 EXIF/IPTC 创建者（无需手动编辑），且已有值或用户已编辑过的作者不会被自动提取覆盖；③多选值不同时 Inspector 显示「多个值」，不可直接改 | 实现：[schema v15](../../src/worker/library-service.ts)（`AUTHOR_METADATA_SCHEMA_SQL`）/ [author-from-exif](../../src/worker/author-from-exif.ts) / [InspectorPanel 作者字段](../../src/renderer/InspectorPanel.tsx)。自动化：[EXIF 提取单测](../../tests/unit/author-from-exif.test.ts) 14/14 / [worker 元数据 get/set/排序](../../tests/worker/organization.test.ts)（作者校验与读写）/ [author 排序 worker 测试](../../tests/worker/search.test.ts)（nulls-last + 大小写不敏感）。人工/平台证据：未执行（Computer Use 当前环境无桌面控制能力，移交人工 QA）。 | 2026-07-18 Serpent-7x0 实现进入待验收；三步操作均需用户实机确认，尤其②的真实 EXIF 自动提取需要一张带真实相机/编辑软件元数据的图片验证（不能只用构造的最小 JPEG fixture）。 |
| META-010 | GIF 时长与帧数元数据 | 待人类验收 | 导入或打开多帧 GIF；等待缩略图生成（可重开资源库触发重排队）；选中该 GIF 查看 Inspector 紧凑信息；对照网格卡片左下角标 | Inspector 在体积/尺寸旁显示时长（如 `0:02`）与帧数（如 `24 帧`）；网格 GIF 卡片在有时长时显示左下时长角标（与 CANVAS-012 一致）；静图不出现帧数 | [开发日志](../development/2026-07-18-gif-controls-metadata-development-log.md) / [gif-metadata 单测](../../tests/unit/gif-metadata.test.ts) / 工单 Serpent-6s1 | 2026-07-18 实现 CU-D8 元数据半边；旧 GIF 会在缩略图入队时因缺 extracted_metadata 失效重生成一次。 |
...
| CANVAS-014 | 网格动图/视频悬停或选中预览 | 待人类验收 | 网格中悬停 GIF、再悬停视频、移开；再单击选中一项 GIF/视频且指针不在卡片上 | 悬停时卡片内播放（GIF 动图 / 视频静音循环）；移开且未选中恢复封面；选中时同样播放；同时只播一个，悬停优先于选中；静图始终仅封面 | [开发日志](../development/2026-07-18-asset-card-hover-preview-development-log.md) / [单测](../../tests/unit/asset-card-hover-preview.test.ts) / 工单 Serpent-05o | 2026-07-18 实现。 |
...
| CANVAS-017 | 递归/混目录视图显示来源文件夹 | 待人类验收 | 进入含子文件夹的父文件夹，开启「包含子文件夹」；观察子级资产卡片左上角 | 子文件夹资产显示紧凑来源路径（相对当前文件夹）；当前文件夹直属资产无角标；「所有资产」/搜索/合集中文件夹资产也显示来源路径 | [开发日志](../development/2026-07-18-recursive-source-badge-development-log.md) / [来源角标单测](../../tests/unit/asset-source-badge.test.ts) / 工单 Serpent-mvn | 2026-07-18 实现 CU-U1；依赖 REQ-FOLDER-009（FOLDER-009 已人类验收通过）。 |
...
| VIEW-010 | 查看页 GIF 暂停与逐帧 | 待人类验收 | 双击多帧 GIF 进入查看页；按空格或点暂停；再播放；若逐帧按钮可用则点上一帧/下一帧 | 空格/按钮可暂停与继续（暂停冻结当前画面）；F 仍回正；ImageDecoder 可用时显示帧号并可逐帧；不可用时逐帧按钮禁用但暂停仍可用；悬停网格预览本条不要求暂停控件 | [开发日志](../development/2026-07-18-gif-controls-metadata-development-log.md) / [单测](../../tests/unit/gif-player-controls.test.ts) / 工单 Serpent-6s1 | 2026-07-18 实现 CU-D8 查看半边；Computer Use 未执行。 |
...
| NAV-006 | 侧栏长名称单行省略 | 待人类验收 | 在左侧目录树准备深层/很长的文件夹名，以及较长的合集与智能合集名；缩窄左栏；悬停被截断的行 | 行高保持单行不因名称折行撑高；末尾省略号；悬停 title 显示全名（链接离线行另附状态说明）；左侧展开/折叠箭头仍可点 | [开发日志](../development/2026-07-18-sidebar-name-ellipsis-development-log.md) / 工单 Serpent-l6v | 2026-07-18 实现（CU-D9）。 |
