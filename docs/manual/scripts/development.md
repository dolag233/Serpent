...
API 版本：`AUTOMATION_API_VERSION = 1`。本文以当前 Console 实际注入的 `src/scripting/serpent-guest-api.ts`、Registry 和 E2E 行为为准。逐项签名见 [脚本 API 参考](api-reference.md)；编辑器类型提示见 [`docs/internal/skills/serpent-automation/automation-api.d.ts`](https://github.com/dolag233/Serpent/blob/dev/docs/internal/skills/serpent-automation/automation-api.d.ts)。类型声明由 Registry 生成/校验，但个别 Registry 命令尚未投影到 Console，见“实现差异”。
