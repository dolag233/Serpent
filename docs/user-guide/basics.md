...
按 `Ctrl+V`（macOS `⌘V`）或使用「编辑」菜单的「粘贴」可从剪贴板导入：剪贴板中是图片时直接导入为新资产；是复制过的文件路径时导入这些文件（可从 Finder/Explorer 复制文件路径后粘贴导入）。剪贴板内容会通过操作系统格式互通（macOS 的 NSFilenames 与 Windows 的 CF_HDROP）。

#### 序列帧导入

拖入或粘贴一组编号帧（如 `00001.png`…`00150.png`）时，Serpent 会识别为序列并弹出序列导入对话框，可设置 FPS。导入后作为一套可播放的资产在查看器中播放，也可以「解散」回单帧。
