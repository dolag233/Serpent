...
可以拖入单个文件、多个文件或文件夹，也可以使用工具栏/主菜单中的「导入文件」「导入文件夹」。文件夹会递归扫描；浏览器扩展还支持把网页图片或视频右键保存、或直接拖拽保存。
...
双击资产打开查看器。图片、SVG、RAW、PSD、TIFF、TGA、EXR 等会通过相应的解码/代理路径显示；SVG 在查看器中按矢量源渲染，而不是把缩略图当作最终图像。视频默认循环播放，必要时由 Serpent 生成兼容代理；音频显示波形并提供播放控制；3D 模型使用独立的模型查看器。

查看器支持平移、滚轮缩放、居中适配（小键盘 `.`）、全屏以及图片/视频的旋转和水平/垂直镜像。非 PNG/JPEG 的图像可能包含文件色彩空间信息；查看器会优先使用读取到的色彩空间，并允许在可用选项间切换。EXR 可在存在多个 plane/part 时选择显示项，但这不是专业通道调色工具。
...
Serpent 支持通过 WebDAV 在多台电脑之间同步资源库。

- **全局服务器（「设置 → 同步」）**：添加、编辑和删除 WebDAV 服务器。地址可以包含路径（如 `https://nas.local/dav/share/Serpent`）；密码保存在本机并加密存储，不会回传明文。保存后会自动测试连接并显示状态（已连接 / 连接失败 / 连接中）。
- **资源库绑定（「资源库设置 → 同步」）**：为当前资源库选择服务器，并设置「同步文件夹名称」（默认使用资源库名称）。最终远端位置为 `服务器地址/同步文件夹名称/`。页面会显示同步状态（未同步 / 正在同步 / 已同步）与服务器连接状态，并提供「预览差异」「立即同步」「保存」。
- **首次同步会把本库的资产、元数据与清单上传到服务器**；之后每次同步只传输有变化的文件。两台设备修改同一文件时，输家版本会保存为「名称 (conflict-…)」的冲突副本，不会静默覆盖。
- **打开同步资源库（「打开资源库」面板 →「打开同步资源库…」）**：选择服务器后列出服务器上已有的同步库，选择一个即可拉取并创建本地资源库。服务器要求支持文件上传/下载（PUT/GET）。资源库名称菜单和主菜单「资源库」不再单独列出此项。
