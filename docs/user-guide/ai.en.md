...
For a manual run, choose **AI analysis** from an asset context menu or use the batch action for a multi-selection. **Analyze unanalyzed assets** skips assets that already have AI content. Choosing the normal action again deliberately re-queues an existing result; a successful run atomically replaces the previous result and does not keep history. When multiple assets are selected, the Inspector also lets you change their ratings in one batch instead of editing each asset.
