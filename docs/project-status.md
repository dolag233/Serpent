...
- **2026-08-01 复验结果**：合集创建焦点、合集删除语义、合集 inline 重命名、文件夹卡片圆角、标签 hover 基础行为和合集递归已通过；submenu 生命周期已通过。硬盘删除资产/含资产文件夹的权限错误（`Serpent-cwor`）仍未解决并重新修复中；删除通知改为“撤销”且恢复后刷新当前范围（`Serpent-mxxc`）待复验；瀑布流 CSS Grid 改动被用户明确拒绝，`Serpent-6b3i` 已恢复原显式列布局并待复验。
...
- **2026-08-01 验收反馈跟进**：合集 inline 重命名/快捷键展示与右键二级菜单互斥已按用户反馈通过；删除通知已改为“撤销”并在恢复后刷新当前范围。硬盘删除的真实 Windows 权限错误（资产及含资产文件夹均高概率出现）已重新打开 `Serpent-cwor`，当前只记录实现与待复验，不写成已解决。瀑布流已用 `git diff` 恢复原有显式列布局，拒绝 CSS Grid 视觉改版；`SELECT-014/015` 等待重新验收。
