...
- 产品负责人新增待办：**ignore**（`Serpent-v6m3`）已完成实现，支持托管/链接文件、文件夹和后缀的持久化忽略、扫描/浏览/搜索过滤、显示隐藏切换、资源库设置及恢复入口；资源库级规则已迁移为可编辑 `.gitignore`（迁移 v26），当前待人类验收；文件夹级删除回归反馈记录为 `Serpent-nplj`。
