...
- 产品负责人新增待办：**ignore**（`Serpent-v6m3`）已完成实现，支持托管/链接文件与文件夹的持久化忽略、扫描/浏览/搜索过滤及设置内恢复入口，当前待人类验收；**常用媒体格式**（`Serpent-aav1`）正在实现。后者已有 Windows 本地 bundle、真实 Worker 解码/代理矩阵和开发态 Electron 媒体预览 E2E 证据，但 Windows/macOS 打包应用的查看/播放与人工验收尚未执行，不能标记完成。
