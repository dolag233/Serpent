...
> 2026-08-22。本文用于规划格式支持工单，不代表下列格式已经实现。格式是否“支持”必须同时验证导入识别、缩略图、查看器解码、格式过滤，以及 Windows/macOS 的实际构建。
...
2. 明确三种能力：源文件查看、缩略图/派生图、播放或交互查看。图像格式通常不需要媒体播放 proxy；只有源文件解码失败时才能进入已有的按需降级路径。
