...
- PDF：原生选择器能找到测试 PDF，但导入按钮保持 disabled；当前媒体注册表也没有 PDF 预览注册，不能写成 PDF 查看器通过。
...
为恢复测试实例曾关闭并重新启动当前 packaged app；没有执行永久删除、清空回收站或从硬盘删除资源库。测试向隔离测试库新增了 2 个视频和 1 个 GIF，最终显示 706 项资产；未自动删除这些测试资产。
