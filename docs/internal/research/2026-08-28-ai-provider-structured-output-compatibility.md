...
> **2026-08-29 修订：** 本文只覆盖了请求侧的结构化输出能力，文中的通用
> `json_schema → json_object → 普通文本 JSON` 结论已被真实回归和进一步调研否定，
> 不应继续作为实现依据。请求能力、响应 envelope、结构化错误和 LM Studio/Qwen
> 的实际返回差异，以 [AI API 请求输出格式与实际响应 envelope 调研](2026-08-28-ai-api-response-envelope-research.md)
> 为准。
