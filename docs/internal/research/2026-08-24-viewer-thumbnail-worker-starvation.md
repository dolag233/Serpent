...
- “赛博配料表”：数据库约 8,863 个资产，其中大部分位于忽略目录，当前可见约一千余项。
- “绘画资源库”：约 7,000 个可浏览资产，派生文件目录约 21,500 个文件。
...
验收目标沿用仓库产品门槛：主窗口壳约 1 秒可交互，查看器/Inspector 首屏约 500 ms 内开始呈现。对本工单建议增加：后台对账期间原生图片/PDF 的 Worker IPC 排队 p95 不超过 100 ms，单次不可抢占同步片段不超过 16 ms。具体阈值应在 Windows 真机基线后固定。
