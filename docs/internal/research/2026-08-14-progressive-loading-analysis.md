...
> 产品修订（2026-08-28）：本文关于资产列表、搜索、缩略图和查看器的渐进式优化仍有效；其中“打开资源库必须永远先显示首屏”的口径已被安全边界产品决定覆盖。资源库打开/切换先给身份与进度，完整结构确认后再显示主界面，详见 [UI direction 0006](../ui/0006-progressive-loading-ux-principles.md)。

> 状态：定稿（四路代码走查完成：文件夹切换 / 搜索 / 封面 / 缩略图队列，均为只读分析）
> 触发：产品负责人反馈——超大型图像测试库下切换文件夹慢、文件夹封面加载慢、
...
- LIB-018（Serpent-tumv，P0 epic）原题为「禁止同步加载：打开资源库约1秒可交互并全面渐进占位」；2026-08-28 已修订为“打开/切换先给身份与进度，结构一致后再显示主界面”，本文其余浏览/搜索/缩略图分析仍作为输入保留——
