...
4. **Desktop 不提供只读资源库**（ADR-0028，2026-08-16）：损坏走备份再 Assets 抢救；新 schema 可写打开。
...
- 仅当宽容读取也无法进行（缺关键表/缺关键列导致核心功能不可用）时，才走**损坏修复**（备份，再 Assets 抢救）或**明确错误**。不打开只读资源库。
...
- 库版本未变 → 下次打开自动重试；同一 from→to 连续失败 **3 次** → 不再自动重试 → **按上次可用 schema 可写打开**（宽容读取优先）。
- 新错误码：`LIBRARY_MIGRATION_FAILED`（已回滚可重试）、`LIBRARY_MIGRATION_STUCK`（连续失败后停在上次可用 schema，仍可写）。
- `verifyMigrationHistory` 失败（checksum 不匹配）：不重试（历史损坏，保持 LIBRARY_CORRUPT，由打开梯度备份/抢救）。
