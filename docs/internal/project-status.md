...
- **2026-09-25 macOS 临时签名**：`osxSign.identity: '-'` 会去钥匙串找名为 `-` 的证书，失败后被 packager 的 `continueOnError` 吞掉，已发布的包只有链接器签名、没有 `_CodeSignature`，隔离后被说成「已损坏」。现关闭身份校验，并在打包校验里要求 `codesign --verify --strict` 通过。ffmpeg、ffprobe、oiiotool 不重复签名，避免改写被锁定的文件字节。临时签名彼此 Team ID 不同，默认加固运行时会在启动时拒绝加载 Electron Framework，系统提示「因为出现问题而无法打开」；授权里已允许加载同包框架。临时签名不消除「无法验证开发者」。0.2.8 的 macOS 安装镜像和便携包已换成这份签名。清单 `MAC-SIGN-001` 用户已确认能进入主界面；隔离后的系统对话框仍未点验。

- **2026-09-25 库内拖拽误显示导入提示**：把当前文件夹里的文件拖走时，系统拖放只带 Files，画布把它当成外部导入。现改为手势真正结束后才收起蓝框，并预热整页、按下时再补预热。清单 `DND-IMPORT-001` 用户已确认通过。WebP 拖拽图标不做转换，系统读不了时仍用应用图标。

- **2026-09-25 文件夹检查器**：浏览某个文件夹且没有选中项时，右侧显示这个文件夹的封面、名称、路径、资产数、子文件夹数和已索引大小；选中画布上的文件夹卡片则改看选中项。合集、标签、搜索、回收站和资源库根目录仍显示资源库摘要。清单 `FOLDER-INSPECT-001` 待人点验。
...
- **2026-09-16 文件夹/合集外观**：第一批已落地。用户确认 APPEAR-001 / APPEAR-002 / APPEAR-003 通过。规格见[外观](implementation/2026-09-16-folder-collection-appearance.md)。工单 `Serpent-df3049`。同步外观另开 P0 `Serpent-d94310`。
- **2026-09-17 链接文件夹展开与排序**：用户确认 `LINKED-TREE-001`、`LINKED-SORT-001` 通过。同层普通文件夹、链接根、虚拟子目录共用名称/数量/时间比较器。见[记录](development/2026-09-16-library-root-linked-tree-expand-development-log.md)。
...
- **2026-09-12 同步搬家留下空目录**：A 移动/重命名已同步文件夹或文件后，B 能同步到新位置，但侧栏仍留着旧的空目录。根因是 `applySyncRelocate` 只搬资产、用文件布局建目标文件夹，不删除已空的源 `managed_folders`。已在回放后按空目录规则剪枝。工单 `Serpent-546f1a`，清单 `SYNC-FOLDER-EMPTY-001`。侧栏点击/拖到空白处回根只记 `Serpent-e71051` / `Serpent-fa76a7`，本轮不实现。
...
- **2026-08-20 PDF 后续页缩放锚点**：Ctrl+滚轮在第一页看起来像以指针为中心，滚到后续页后偏差很大。根因是页列用常数 `gap`/`padding`，且缩放时未先把视口外页盒改到新尺寸，却按「从内容原点整体乘比例」恢复滚动。已改为页内局部锚点。见 [开发日志](development/2026-08-20-pdf-pointer-zoom-anchor-development-log.md)。
...
- **2026-08-16 `Serpent-ot5r` / EXTLIB-002**：打开/导入面板 Billfish 入口取消置灰，入口可用性已由用户确认；当前新增的 `.BillfishPack` 解压、临时目录清理以及 Eagle ZIP/RAR 等归档来源仍待完整人类验收。
...
- **2026-08-15 瀑布流截断白区（Serpent-1s3d）**：根因确认是滚动反向时
  `scroll → rAF → setState` 让旧切片在新视口已经绘制后仍存活，`spacerBefore` 因此落到
  视口下方；3 屏/8 卡的跑道不足以覆盖一次快速反向跳跃。滚动监听现在直接发布最新窗口，
  跑道提高为 `max(1200, 视口×5, 卡片×12)`，仍保留窗口化和统一槽位高度。
  定向 E2E、typecheck、lint 通过；macOS Computer Use 在 477 项资源库的 Value:3/2/1
  各执行 10 次快速滚动并以 10fps 采集画面，未见截断白区。2026-08-15 用户确认至少 10 次快速滑动
  未出现截断式问题，CANVAS-037 人类验收通过；Windows/packaged 未执行。
- **2026-08-13 链接文件夹层级与导入卡顿**：`Serpent-a9vh` 用路径前缀虚拟子目录
  （`lfv:{root}/{path}`）让链接文件夹按磁盘层级出现在侧栏与画布；导入后不再显示
  操作提示文案。缩略图/色板/OIIO 不再用 `asset.changed` 全量刷新网格，
  `library.changed` 也不再重载资产列表（`Serpent-yti0`）。
...
v0.1.0 继续收口 0001–0010 的桌面主线，并纳入真实使用反馈确认的 0013–0019 产品化范围。0011 通用 CLI 已撤回，不属于当前或 v0.2.0 的实现入口；自动化以 Desktop Console、受控脚本和嵌入式 HTTP MCP 为准。0012 已完成 macOS 开发态验收；0014 形成候选 `f1330a7`，但新反馈确认 Inspector、框选修饰键、瀑布流、完整文件菜单、应用壳、目录浏览、标签、双语和主题仍属于 MVP 待办。
