...
- 同 ID 用户级/资源库级冲突必须显式选择；覆盖安装后旧 Resolution 不自动继承，必须重新显式选择当前包；资源库包未信任时返回等待信任状态；Safe Mode 会禁用解析而不删除包。
- 每个安装范围按插件 ID 保留一个活动包目录；新版本经 staging 和二次校验后原子覆盖 `<pluginId>/`，版本信息只在 manifest/lock 中保存。卸载先从 lock 脱离，再删除目录，崩溃最多留下不可激活的孤儿文件；覆盖后旧版本需重新安装，不能依赖本地 rollback 恢复。
- Package lock 现在也保存不可变来源元数据：GitHub 的仓库、tag、commit 与本地来源类型可安全显示给 Renderer；本地绝对路径仍只留在 Main 的原生选择器内。GitHub commit 不再被误当作“来源变化”，但覆盖安装后的旧 Resolution 仍需用户显式选择当前包。
...
- 覆盖安装后如果本设备 Resolution 仍指向旧 package hash，`resolve` 返回 `selected-package-unavailable` 并要求显式选择当前包；本地 rollback API 保留稳定错误契约，但不再假定安装根有上一版本目录。需要回退时重新安装旧版本包。
...
- `PluginSettingsStore` 提供非秘密设置的三层原子 JSON 存储：用户默认值、资源库 `.serpent/plugin-settings/` 值与按资源库哈希分区的本机覆盖值按顺序合并。它只接受清单已声明且类型匹配的基础设置值；信任、隔离、设备 ID 与本机覆盖永远不写入资源库。生成设置 UI、Plugin SDK 的 Storage/Secrets API 仍留给 Phase C。
...
| 复制资源库的设置同步、信任不传播 | `plugin-settings-store.ts`、`.serpent/plugin-settings/` | `plugin-settings-store.test.ts`（两份 userData + 复制库 + 已安装包） | 无真实两设备平台证据；测试不读取真实用户库 |

- `npm run test:unit -- tests/unit/plugin-settings-store.test.ts`：206 个测试文件通过，1,627 个测试通过，1 个跳过。
...
3. Host 生命周期中的生成设置 UI、命名空间 Storage 与系统凭据库 Secrets API 尚未实现；当前仅完成由 Host 调用的非秘密设置存储与同步边界。
