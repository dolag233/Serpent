...
见 `docs/internal/qa/human-acceptance-checklist.md`：`IMAGESEQ-001`～`IMAGESEQ-005` 均已**人类验收通过**（2026-07-28）。

**P3 跟进**（`Serpent-1y9r`）：导入完成 Toast 按 1 套逻辑资产计数（与回收站删除/文件夹计数一致），当前仍可能按帧数提示。

## 建议复验素材

隔离序列帧测试目录：拖 `_00010` → 确认窗 → 整段 0~150、30 FPS、一名 `…_00000~00150`；删序列卡不泄漏单帧；查看页可播。
