...
- Computer Use 在真实 Electron 中创建独立资源库、导入图片、创建标签/合集、分配资产、编辑并清空元数据、验证非法色卡原因；截图见 `docs/internal/qa/evidence/0004-organization-metadata/`。
- 全量 E2E 首轮发现未指定 profile 的进程会争用开发者 `userData/SingletonLock`。Main 在 `SERPENT_E2E=1` 时改用按 PID 唯一的临时 profile；显式重启/单实例用例仍可传入同一隔离路径。流程规则已写入 `docs/internal/development-process.md`。
