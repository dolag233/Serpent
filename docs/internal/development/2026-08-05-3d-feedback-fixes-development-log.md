...
设计产出：`docs/internal/implementation/0031-schema-compatibility-guarantee.md`（v3）。产品负责人方向：**不依赖备份**——主防线 = 高版本完全兼容低版本数据（迁移无损 + 全版本链测试）+ **鲁棒错误恢复**；**鲁棒性 = 结构容错（宽容读取）**：表缺列/多列时忽略差异、读取剩余信息，不因结构差异直接失败。设计三块：① 宽容读取（列探测缓存 + 读路径白名单化 + 缺列默认值降级，版本门禁降为最后手段）；② 迁移原子性 + 失败诊断重试（migration-failed.json、3 次上限、只读降级兜底）；③ 全版本链兼容测试 + 结构变异测试 + 迁移纪律静态检查。实施拆 8 子任务（实施交接）。
