...
### 9.1 Release note 重写（用户反馈：上一版面向开发者，不合格）

用户指出三条通病，已按此重写 Release 正文与 `release-meta`（内容变了，资产用 `--clobber` 覆盖重传）：

| 问题 | 改法 |
| --- | --- |
| 条目标题不像人话、正文把组件逐个背诵（如「字体资产：卡片看得到字形，打开能试排版」） | 标题改成动宾短句（「支持字体预览」「文件夹批量操作」「支持跳转到文件所在文件夹」「文本自动换行」…），正文只写**用户能做什么 + 一句话说明**，不再列举 UI 组件；UI 微调用概括语句 |
| 口语化标题（如「序列帧更好管」） | 改为「序列帧导入管理」等中性表述，全文去掉口语 |
| 把新功能写进「体验与性能」、并暴露快捷键（「Ctrl+B 跳到库内所在文件夹」） | 「支持跳转到文件所在文件夹」归入**新功能**；正文不出现任何快捷键与内部实现（数据来源：`docs/internal/development/2026-09-19-color-filter-folder-tree-tab-preview-development-log.md` 等三篇日志） |

同时补齐用户点出的缺漏条目：**新功能**「文本自动换行」（默认开启，可关掉恢复横向查看）；**修复**「勾选格式过滤『文本』后出现内部错误」（Renderer 只发压缩令牌、Worker 展开，格式上限与标签上限分开）、「颜色过滤只看色相」（改为以目标色为中心的 HSL 盒子，黑白与灰色不再被误判成红色）、「打开资产查看器后切换标签页再回来显示错误视图」（按 tabId 往回找非 preview 条目恢复浏览范围）。

**更新命令与结果**：`gh release edit v0.2.6 --notes-file <临时草稿>` → 正文已更新；`gh release upload v0.2.6 --clobber release-meta.json release-meta-0.2.6.json` → 两个元信息资产均变为 1794 B（新内容）；发布资产仍为 10 个（Windows 4 + macOS 4 + release-meta 2）。
