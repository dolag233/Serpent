...
- `.asset-caption` 的上下内边距统一为 4px，并显式固定分辨率、文件名和元数据行高，保证首行与末行的视觉间距可比较。
- caption 使用垂直居中分配预留高度；字体栅格化或缩放造成的剩余行盒空间会均分到上下，避免底部单侧累积空白。
- `asset-caption-band`、`justified-caption-band` 与画布布局常量同步更新，瀑布流两行/三行标题区分别为 37/54px，平铺三行标题区为 54px；两种视图使用同一组行高、内边距和 3px 行间距，虚拟布局与实际 CSS 保持一致。
