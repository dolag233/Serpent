...
| 悬停 500ms 延迟打开，离开取消，键盘聚焦即时 | `src/renderer/DimensionFilterBar.tsx` | `tests/unit/dimension-filter-bar.test.tsx`：1/1 | Computer Use 待执行；Windows、触控/高 DPI 未验证 |
...
| 检查 | 命令/结果 |
| --- | --- |
| Renderer 定向单测 | `npx vitest run --config vitest.config.ts tests/unit/filter-tag-picker.test.tsx tests/unit/dimension-filter-bar.test.tsx`：2 files，2 tests passed |
...
| 资源库可用性 / 真实 Electron / Computer Use | 待最终收口执行 |
