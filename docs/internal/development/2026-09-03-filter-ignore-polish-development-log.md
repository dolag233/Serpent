...
- `Serpent-77c919`：标签过滤同时显示最近筛选和全部标签；最近区最多两排，全部标签区最多六排并独立滚动，已选标签使用主题强调色高亮。
...
3. 标签过滤器复用既有 `sortTags`、`dimension-filter-btn` 和排序 glyph；默认使用次数降序，同一字段再次点击切换方向，名称切换时默认为升序。默认列表拆分为最近筛选和完整标签集合：最近区最多两排，全部标签区最多六排并独立纵向滚动；已选标签 chip 使用主题强调色高亮。
...
| 标签过滤显示最近/全部、独立滚动、已选高亮 | `src/renderer/FilterTagPicker.tsx`；`src/renderer/styles.css` | `tests/unit/filter-tag-picker.test.tsx`：2/2；`tests/e2e/browsing-preferences.test.ts`：真实面板与滚动断言 | Computer Use：macOS 真实应用已检查最近区、全部标签区、独立滚动、底部提示和已选 chip 高亮；Windows、窄窗口和高 DPI 未验证 |
...
## 用户反馈：已选标签高亮（2026-09-03）

用户确认主题色通过，并补充要求标签过滤面板中的已选标签需要高亮。Computer Use 复现显示已选“表情包”此前与普通标签一样是中性灰色 chip，无法快速辨认当前筛选条件。

修复为给已选 chip 增加 `.is-selected` 语义类，背景、边框、文字和移除按钮统一复用 `--accent-soft`、`--accent-soft-fg` 与现有强调色边框 token；未选标签和过滤逻辑不变。定向单测新增选中 chip 结构断言，Electron E2E 新增真实已选标签高亮断言。Computer Use 复核中选中 chip 已随当前主题色显示高亮，等待用户确认标签体验；Windows、packaged、真实 DPI 未验证。

补充回归结果：`npm exec vitest run --config vitest.config.ts tests/unit/filter-tag-picker.test.tsx` 为 1 file / 2 tests passed；`node scripts/run-e2e.mjs tests/e2e/browsing-preferences.test.ts` 为 4 tests passed；当前 HEAD 的 `npm run test` 为 508 files / 4388 tests passed、15 files / 25 tests skipped。标签工单仍保持 in_progress，等待用户最终验收。
