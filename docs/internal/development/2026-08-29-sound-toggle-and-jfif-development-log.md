...
- 播放入口在创建音频对象前检查偏好，因此关闭后不会创建或播放音频。提示音只接入导入、导出、同步、资源转换和批量文件操作等可能长时间运行的任务，并且只有实际耗时严格超过 1 分钟才播放；打开、关闭或切换资源库本身不播放，取消也不播放。
...
新增了面向软件开发者的 [媒体格式支持指南](../../../developer/media-format-support.md)，并从中英文开发者文档入口链接。指南把共享注册表、导入/解码/查看器/格式过滤/插件 MIME 等消费者和测试矩阵列成逐项清单，避免只在某一个扩展名列表里加格式。
