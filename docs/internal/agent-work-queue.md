...
| **需求来源** | `docs/internal/implementation/mvp-ui-ux-requirements-backlog.md` | 用户原话、REQ ID、产品方向 | 理解背景；无工单则开单 |
| **功能验收清单** | `docs/internal/qa/human-acceptance-checklist.md` | UI：人类操作通过/不通过；**PLUGIN/AUT：自动化验收** | UI 不从「待人类验收」当执行队列；PLUGIN/AUT 按测试档位更新状态；「人类验收不通过」仍是缺陷池 |

**UI「待人类验收」条目多是正常现象**：值得人点，不等于未实现。  
**插件开发**：见 [`docs/internal/agent-plugin-playbook.md`](agent-plugin-playbook.md)。
...
1. 读 `docs/internal/project-status.md` 的 **「当前前沿」**（发布阻断 vs MVP 产品化）。
...
```bash
rg '\| 人类验收不通过 \|' docs/internal/qa/human-acceptance-checklist.md
```
...
```bash
rg '未实现|待实施|待做' docs/internal/implementation/mvp-ui-ux-requirements-backlog.md
```
...
`docs/internal/project-status.md` → **当前前沿 §1 P0**：0006 打包媒体二进制、Windows runner、`verify:mainline`。这类工作往往 **跨切片**，需在 `docs/internal/implementation/0006-*.md` 等规格里拆工单。
...
# 清单待验数量（信息用，非队列长度）
rg -c '\| 待人类验收 \|' docs/internal/qa/human-acceptance-checklist.md
