...
0011 不作为 `v0.1.0` 首发 MVP 的发布阻断，完整能力仍作为 `v0.2.0` 主切片。现在允许先并行实施
CLI 契约、共享 command registry/schema、机器可读帮助和不修改资源库的只读入口；导入/修改命令、
跨进程写租约、持久变更序号、detached job 领取和双平台独立分发，仍需等 v0.1.0 领域语义与发布收口
后实施。这样既响应脚本化优先级，也避免在底层工作流仍变化时固化两套写入语义。
...
- 资产信息：真实文件名、描述、评分、喜欢、源链接、人工色卡；CLI 不提供独立 Label/显示别名。
- 标签：CRUD、分配与移除。
