...
- **文案面向用户（强制，2026-09-12 用户反馈）**：设置、按钮、空状态、提示只写用户会遇到的结果和可做的选择。不得把规格条目、实现约束或内部讨论写进界面，也不要把界面行为已经能看见的事再解释一遍。详见 `docs/internal/ui/0004-calm-error-and-copy-ux-principles.md` §6。（镜像 AGENTS.md）

- **先复用后新建（强制，2026-09-05 用户反馈）**：添加任何 UI 或功能之前，先快速检查项目内是否已有类似功能/组件/实现；若已有，直接复用或在既有能力上扩展，禁止另起炉灶照抄一份。先例：侧栏「文件夹树排序」曾自造 6 向扁平选项菜单，与既有资产排序控件（SortModeControl）的「方向单选组 + 字段列表」结构不同，返工对齐。功能/UI 审查固定项：落地前核对一次「项目里是否已有等价物」并说明复用点。（镜像 AGENTS.md）
